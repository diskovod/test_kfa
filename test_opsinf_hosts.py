...
class Test_NetworkDeviceTests:
	...
	loadSSHkey(pytest.config.getoption("--key"))
	...
	# Get device groups
	groups = getGroups(c, device_name, test_ids, 'host') # TODO: Fix overkill group filtration at this lvl
	#print groups
	...
	devices, gen_simple, gen_simple_w_setup, test_cases_grp, test_cases_uni = loadTestCases(c, groups, test_ids, device_name, dc_location, 'host')
	...
	TEST_LOG['devices'] = json.dumps([device['host_shortname'] for device in devices])
	...
	def test_generic_simple(self, input, output, device, host, url, id, descr):
		...
		loadShortName(device.split('|')[-1].strip())
