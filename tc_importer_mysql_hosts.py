...
template_dict = {
	...
	'data'   :  {
					'input'  : { 'db_map' : {'db' : "testcase_data_host", 'field' : "input_data"} }, 
	            	'output' : { 'db_map' : {'db' : "testcase_data_host", 'field' : "output_data"} },
	            	'name'   : { 'db_map'   : {'db' : "testcase_data_host", 'field' : "descr"} }
	            },
	...
	'device' :  {
					'group' : { 'db_map' : {'db' : "host_group", 'field' : "host_keyword"} },
					'name' : { 'db_map' : {'db' : "host_list", 'field' : "host_shortname"} }
				}
...
c.execute('SELECT tims_id FROM testcase_data_host')
tc_in_db = c.fetchall()
filtered_urls = []
				...
				if value in template_dict[option]:
					...
					if option == 'data':
						...
						if fl_in == 0:
							...
							SQL_UPDATE = 'INSERT INTO '+template_dict[option][value]['db_map']['db']+' ( setup_id, grp_id, in_group, host_shortname, tims_id, %s ) VALUES( %%s, %%s, %%s, %%s, %%s, %%s, %%s, %%s )'
	...
	for order in sorted(OPERATIONS_DICT):
		...
		if order == '2':
			...
			if 'name' in test_dict['device']:
				...
				#if type(test_dict['device']['name']) != list:
				c.execute('SELECT host_shortname FROM host_list WHERE host_shortname = %s ',  [ test_dict['device']['name'] ] ) # TODO: not working now
				
				if c.fetchone() is not None:
					fl_uniq_device = True
				#else:
				#	c.execute('SELECT host_shortname FROM host_list WHERE host_shortname IN (%s) ',  [ ','.join([ '"'+e+'"' for e in test_dict['device']['name'] ]) ] )
				#	print c.fetchall()
